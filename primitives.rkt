...
(define/match (((↑↓-core char slice) [undo #f]) . args)
...
(define/match (((«»-core char) [undo #f]) . args)
...
(define ((BQN! [undo #f]) x [w "Assertion error"])
  (if (equal? x 1)
      1 (error w))
  )
